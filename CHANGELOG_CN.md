...
* 注意：根据我们的 SBE 政策，[在被废止 6 个月后](faqs/sbe_faq_CN.md#sbe-schema)， SBE 3：2 模式将于 2026 年 09 月 25 日被停用。
...
* 注意：根据我们的 SBE 政策，[在被废止 6 个月后](faqs/sbe_faq_CN.md#sbe-schema)， FIX SBE 1：0 模式将于 2026 年 09 月 25 日被停用。
* [面向生产的 FIX SBE 生命周期](./sbe/schemas/sbe_fix_schema_lifecycle_prod.json) 已基于本次更改进行了更新。
...
* 从 [QuickFIX 订单接入 schema](./fix/schemas/spot-fix-oe.xml) 和 API 文档中的 `ListStatus <N>` 移除了顶层字段 `Symbol (55)`。正如[此前公告](#2025-12-02)中所述，API 已不再发送该字段。

---

### 2026-07-27

* 更新了 [价格区间执行规则 FAQ](./faqs/price_range_execution_rules_CN.md#externalCalculationId4)，新增了外部参考价格计算方法。
...
* 自 **2026-07-09 07:00 UTC** 起，[WebSocket 行情接口](web-socket-streams_CN.md) 将进行基础设施升级，整个升级过程最长耗时 1 小时。
* 升级期间，您的 WebSocket 连接可能会断开；如发生此情况，请重新连接。
...
* 注意：根据我们的 SBE 政策，[在被废止 6 个月后](faqs/sbe_faq_CN.md#sbe-schema)， SBE 3：1 模式将于 2026 年 06 月 29 日被停用。
* [面向生产的 SBE 生命周期](https://github.com/binance/binance-spot-api-docs/blob/master/sbe/schemas/sbe_schema_lifecycle_prod.json) 已基于本次更改进行了更新。
...
* 中文文档术语统一与文字校对：
  * 统一中文术语：`手续费`/`交易费` → `佣金`，`智能指令路由` → `智能订单路由`，`WebSocket 账户信息推送` → `用户数据流`，`订单薄` → `订单簿` 等。
  * 修正了部分文档中的措辞与排版问题。
...
以下事项将于 **2026年5月5日 大约10:00 UTC** 发生。
...
* 更新了文档，明确说明了如果某个交易对从未设置过参考价格，那么查询参考价格时就会返回错误代码 [`-2043`](errors_CN.md#-2043-no_reference_price)。此规则适用于以下接口/方法：
...
* 补充了有关 [价格区间执行规则](./faqs/price_range_execution_rules_CN.md#price_range_enforcement) 中规则执行的更多信息。
...
* 更新了[价格区间执行规则](./faqs/price_range_execution_rules_CN.md#execution_price_limits)，以说明订单的执行价格限制。
...
* 在[价格区间执行规则](./faqs/price_range_execution_rules_CN.md#externalCalculationId0) 中新增了外部参考价格计算方法。
...
* 在 [FIX SBE 文档](fix-api_CN.md#fix-sbe) 中阐明了有关指数字段的要求
...
* `recvWindow` 上的变更将被启用。
  ...
  * `recvWindow` 支持微秒级。
    ...
    * 受影响的 API：
      ...
      * REST API
      * WebSocket API
* 新的 [`MAX_NUM_ORDER_LISTS`](filters_CN.md#max_num_order_lists) 过滤器将被启用：会允许每个交易对上最多有 20 个订单列表挂单。
